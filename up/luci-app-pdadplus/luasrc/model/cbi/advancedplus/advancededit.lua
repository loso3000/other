...
if nixio.fs.access("/etc/config/advancedplus")then
s:tab("advancedplusconf",translate("advancedplus"),translate("本页是配置/etc/config/advancedplus的文档内容。应用保存后自动重启生效"))
conf=s:taboption("advancedplusconf",Value,"advancedplusconf",nil,translate("开头的数字符号（＃）或分号的每一行（;）被视为注释；删除（;）启用指定选项。"))
conf.template="cbi/tvalue"
conf.rows=20
conf.wrap="off"
conf.cfgvalue=function(t,t)
return e.readfile("/etc/config/advancedplus")or""
end
conf.write=function(a,a,t)
if t then
t=t:gsub("\r\n?","\n")
e.writefile("/tmp/advancedplus",t)
if(luci.sys.call("cmp -s /tmp/advancedplus /etc/config/advancedplus")==1)then
e.writefile("/etc/config/advancedplus",t)
luci.sys.call("/etc/init.d/advancedplus restart >/dev/null")
end
e.remove("/tmp/advancedplus")
end
end
end
