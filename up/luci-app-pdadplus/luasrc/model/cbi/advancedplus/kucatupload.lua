...
ful = SimpleForm('upload', translate('Upload  (Free: ') .. wa.byte_format(free_byte) .. ')', translate("You can upload files such as jpg,png,gif files, To change the login page background."))
ful.reset = false
ful.submit = false

sul = ful:section(SimpleSection, '', translate("Upload file to '/www/luci-static/kucat/background/'"))
...
local dir, fd
dir = '/www/luci-static/kucat/background/'
nxfs.mkdir(dir)
http.setfilehandler(
    function(meta, chunk, eof)
        ...
        if eof and fd then
            fd:close()
            fd = nil
            um.value = translate('File saved to') .. ' "/www/luci-static/kucat/background/' .. meta.file .. '"'
        end
    end
)
