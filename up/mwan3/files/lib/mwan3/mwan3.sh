...
mwan3_create_iface_route()
{
	...
	mwan3_get_routes | while read -r route_line; do
		...
		if [ -z "$tid" ] || [ "$tid" = "$id" ]; then
			...
			$IP route add table $id $route_line || LOG debug "Route '$route_line' already added to table $id"
		fi
...
mwan3_report_policies()
{
	...
	total_weight=$($ipt -S "$policy" | grep -v '.*--comment "out .*" .*$' | cut -s -d'"' -f2 2>/dev/null | head -1 | awk '{print $3}')

	if [ -n "${total_weight##*[!0-9]*}" ]; then
		for iface in $($ipt -S "$policy" | grep -v '.*--comment "out .*" .*$' | cut -s -d'"' -f2 2>/dev/null | awk '{print $1}'); do
			weight=$($ipt -S "$policy" | grep -v '.*--comment "out .*" .*$' | cut -s -d'"' -f2 2>/dev/null | awk '$1 == "'$iface'"' | awk '{print $2}')
