...
conf.wrap = "off"
function conf.cfgvalue(self, section)
    return fs.readfile("/etc/appfilter/feature.cfg.user") or ""
end
function conf.write(self, section, value)
    if value then
        value = value:gsub("\r\n?", "\n")
        fs.writefile("/tmp/feature.cfg.user", value)
        if (SYS.call("cmp -s /tmp/feature.cfg.user /etc/appfilter/feature.cfg.user") == 1) then
            fs.writefile("/etc/appfilter/feature.cfg.user", value)
	     SYS.exec("chmod 666  /etc/appfilter/feature.cfg.user 2>/dev/null")
	     SYS.exec("rm -rf /tmp/appfilter 2>/dev/null")
	     SYS.call("/etc/init.d/appfilter restart >/dev/null")
        end
        fs.remove("/tmp/feature.cfg.user")
    end
end
