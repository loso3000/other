...
function index()
	...
	local e = entry({"admin", "control", "appfilter"}, alias("admin", "control", "appfilter", "base_setting"),_("App Filter"), 20)
    e.dependent=false
    e.acl_depends = { "luci-app-oaf" }
	...
	page = entry({"admin", "network", "apprefresh"}, call("refresh_data"))
	page.leaf = true
...
function refresh_data()
	local e = {}
	e.ret = luci.sys.call("/usr/bin/appfilterupdate > /dev/null 2>&1 &") == 0
	e.retcount = tonumber(luci.sys.call("cat /tmp/feature.cfg | grep -v '#'|awk NF|wc -l"))
